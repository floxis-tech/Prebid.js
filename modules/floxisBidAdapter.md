...
The adapter reports client-observed auction timeouts and bidder transport errors to Floxis as cookieless operational telemetry. Beacons carry only the seat, region, event type, and relevant operational dimensions (HTTP status, timeout flag, duration, auction ID, page URL) — no user or device identifier is included. Consent signals are forwarded as opaque pass-through parameters where available. Each beacon fires at most once per distinct seat+region pair per event, and telemetry failures are silently suppressed so they never affect the auction lifecycle.
